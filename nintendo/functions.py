...
def create_text2_without_hash_ats_links(df1):
    ...
    words_to_remove_lists = (
        [unique_hashtag_list(df1)] + [unique_link_list(df1)] + [unique_ats_list(df1)]
    )
    words_to_remove = []
...
def lemmatize_text(df1):
    ...
    for tweet in df1["text2"]:
        lemmatized = [
            lemmatizer.lemmatize(tweet, get_wordnet_pos(tweet))
            for tweet in nltk.word_tokenize(tweet)
        ]
...
def dict_by_index_of_filtered_times(
    df1, start_time_str="16:07:24", end_time_str="16:10:46"
):
    # print(start_time_str, end_time_str)
    every_index = df1["index"]
    # convert input strings to datetime.time
    start_times = datetime.datetime.strptime(start_time_str, "%H:%M:%S")
    start_times = start_times.time()
    end_times = datetime.datetime.strptime(end_time_str, "%H:%M:%S")
    end_times = end_times.time()
    # filters by time (returns in True/False)
    specific_times = (df1["datetime"] > start_times) & (df1["datetime"] <= end_times)
...
def trend_line_by_time(
    df1, start_time_str="16:07:24", end_time_str="16:10:36", sum_mean="sum"
):
    """Returns a trend line using start and end times"""
    criteria = dict_by_index_of_filtered_times(
        df1, start_time_str=start_time_str, end_time_str=end_time_str
    )
...
def vectorize_to_matrix_df(df1):
    ...
    matrix_df = pd.DataFrame(
        word_array, columns=unique_words_list(df1), index=df1.index
    )
    return matrix_df
...
def words_df(df1):
    ...
    df = df1.rename(
        index=str, columns={"text": ".text.", "lang": ".lang.", "time": ".time."}
    )
...
def create_dictionary_for_specified_time(
    df1, which_five="top"
):  # time=1, seconds=5, which_five='top'): # choose either 'top' or 'bottom'
    ...
    df_filtered_by_temp_col = df1.loc[
        (df1["temp_criteria_col"] == 1)
    ]  # | (df_words['five_seconds']== 2)]
    ...
    no_dupl_list_of_word_dicts = [
        i
        for n, i in enumerate(list_of_word_dicts)
...
def top_5_dict_to_df(df1, which_five="top"):
    ...
    top_5_df = pd.Series(
        create_dictionary_for_specified_time(df1, which_five=which_five)
    )
...
def output_radar_plot(df1, which_five="top"):
    ...
    # Set data
    radar_df = top_5_dict_to_df(df1, which_five=which_five)
    ...
    dictionary = create_dictionary_for_specified_time(df1, which_five=which_five)
    list_of_scores = []
    ...
    plt.yticks(
        [0, (relevant_score * 0.33), (relevant_score * 0.66), (relevant_score)],
        [0, "", "", ""],
        color="grey",
        size=7,
    )
...
def automated_radar_plots(
    df1, start_time="16:07:24", end_time="16:08:36", which_five="top"
):
    """Turns original df into radar plot."""
    criteria = dict_by_index_of_filtered_times(
        df1, start_time_str=start_time, end_time_str=end_time
    )
    if "temp_criteria_col" not in df1.columns:
        pass
    else:
...
# def import_df_pickles():
#     with open ('./nintendo/pkl/words_df_pickle_1.pkl', 'rb') as f:
#         df_pkl_1 = pickle.load(f)
#     with open ('./nintendo/pkl/words_df_pickle_2.pkl', 'rb') as f:
#         df_pkl_2 = pickle.load(f)
#     with open ('./nintendo/pkl/words_df_pickle_3.pkl', 'rb') as f:
#         df_pkl_3 = pickle.load(f)
#     with open ('./nintendo/pkl/words_df_pickle_4.pkl', 'rb') as f:
...
#                     df_pkl_5.reset_index(drop=True),
#                     ], axis=1)
#     return df_c
